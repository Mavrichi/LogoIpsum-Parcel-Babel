...
async function runPackage(workerApi, {
  ...
  let config = await runner.loadConfig(bundleGraph, bundle); // TODO: add invalidations in `config?.files` once packaging is a request

  let cacheKey = await runner.getCacheKey(bundle, bundleGraph, config === null || config === void 0 ? void 0 : config.config);
  let cacheKeys = {
  ...
  return (_await$runner$getBund = await runner.getBundleInfoFromCache(cacheKeys.info)) !== null && _await$runner$getBund !== void 0 ? _await$runner$getBund : runner.getBundleInfo(bundle, bundleGraph, cacheKeys, config === null || config === void 0 ? void 0 : config.config);
}