...
class PackagerRunner {
  ...
  async getBundleInfo(bundle, bundleGraph, cacheKeys, config) {
    let {
      type,
      contents,
      map
    } = await this.getBundleResult(bundle, bundleGraph, config);
    return this.writeToCache(cacheKeys, type, contents, map);
  }

  async getBundleResult(bundle, bundleGraph, config) {
  ...
  async package(internalBundle, bundleGraph, config) {
  ...
  async getCacheKey(bundle, bundleGraph, configResult) {
